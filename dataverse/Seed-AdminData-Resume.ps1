...
<#
.SYNOPSIS
  Resume del seed — secciones 4-9 (Transitions, AppUsers, RBAC, RolePerms, Settings, WIP).
  Las secciones 1-3 (BusinessAreas, Teams, States) ya se insertaron en la ejecucion anterior.
  Este script recupera los GUIDs de los estados desde Dataverse y continua desde las transiciones.
#>
param(
...
Write-Host "[AUTH] Device-code para $OrgUrl ..." -ForegroundColor Cyan
...
try { $token = Get-MsalToken @tokenParams }
catch {
...
# ============================================================================
# RECUPERAR GUIDs DE STATES YA CREADOS
# ============================================================================
Write-Host "=== Recuperando GUIDs de States desde Dataverse ===" -ForegroundColor Cyan
$statesResp = Invoke-RestMethod -Uri "$apiBase/cproroad_states?`$select=cproroad_stateid,cproroad_name" -Headers $hdr
$stateGuid  = @{}
...
# 6. RBAC PERMISSIONS
# ============================================================================
Write-Host "=== 6. RBAC Permissions ===" -ForegroundColor Cyan
$rbac_data = @(
    ...
    @{ key="TRANS_RFT_TEST";   label="LISTO PRUEBAS -> EN VALIDACION";    group="TRANSICIONES" },
    @{ key="TRANS_TEST_CLS";   label="EN VALIDACION -> CERRADO";          group="TRANSICIONES" },
    ...
    @{ key="VIEW_AUDIT";       label="Auditoria";                          group="VISTAS" },
...
Write-Host "  -> $total entradas rol x permiso creadas`n" -ForegroundColor Green
...
Write-Host "  -> 1 registro de configuracion creado`n" -ForegroundColor Green

# ============================================================================
# 9. WIP CONFIG
# ============================================================================
Write-Host "=== 9. WIP Config ===" -ForegroundColor Cyan
$wipLimits = @{ "En curso"=5; "Bloqueado"=3 }
$i = 0
foreach ($stateName in $wipLimits.Keys) {
    $stateGuidVal = $stateGuid[$stateName]
    if (-not $stateGuidVal) { Write-Warning "Estado '$stateName' sin GUID — omitiendo"; continue }
    $i++
    $limit = $wipLimits[$stateName]
    Progress "WIP" $i $wipLimits.Count "$stateName (limite=$limit)"
    DvPostReturnId "cproroad_wipconfigs" @{
        cproroad_name                   = "WIP-$stateName"
        cproroad_limit                  = $limit
        "cproroad_stateid@odata.bind"   = "/cproroad_states($stateGuidVal)"
    } | Out-Null
}
Write-Host "  -> $i limites WIP creados`n" -ForegroundColor Green
